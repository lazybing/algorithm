class Solution {
    public:
        void solve(vector<vector<char>>& board) {
            ...
            for (int j = 0; j < col; j++) {
                ...
                if (board[row - 1][j] == 'O') dfs(board, row - 1, j, visited, false);
            }

            for (int i = 0; i < row; i++) {
                if (board[i][0] == 'O') dfs(board, i, 0, visited, false);
                if (board[i][col - 1] == 'O') dfs(board, i, col - 1, visited, false);
            }
    ...
    private:
        void dfs(vector<vector<char>& board, int row, int col, vector<vector<int>>& visited, bool filp) {
